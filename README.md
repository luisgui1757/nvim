...
| lazygit Ctrl+J / Ctrl+K acts like Enter inside psmux | psmux v3.3.4 does not relay Windows Terminal's Win32-input-mode modifier data into panes | run `.\bootstrap.ps1` so `%USERPROFILE%\.tmux.windows.conf` root-binds C-j/C-k to F8/F7 and `%LOCALAPPDATA%\lazygit\config.yml` binds move-commit to `<f8>`/`<f7>`. Outside that psmux rescue path, F8/F7 are the configured move-commit keys |
