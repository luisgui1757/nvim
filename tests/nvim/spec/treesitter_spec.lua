...
describe("treesitter ensure_installed", function()
  ...
  for _, lang in ipairs(required) do
    it("includes " .. lang, function()
      assert.is_truthy(src:match('"' .. lang .. '"'), lang .. " missing from ensure_installed")
    end)
  end
  ...
  for _, lang in ipairs(forbidden) do
    it("does not include " .. lang, function()
      assert.is_nil(src:match('"' .. lang .. '"'), lang .. " should be removed from ensure_installed")
    end)
  end

  it("does not force a specific compiler (let TS auto-detect)", function()
    assert.is_nil(src:match("install%.compilers"), "don't pin compilers — clang fails on Linux/Windows without it")
  end)
