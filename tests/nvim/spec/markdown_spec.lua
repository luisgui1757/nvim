...
describe("markdown rendering plugin", function()
	...
	it("lazy-loads on markdown ft only", function()
		assert.is_truthy(src:match('ft = { "markdown"'),
			"render-markdown should only load when entering a markdown buffer")
	end)

	it("enables task-list checkbox rendering", function()
		assert.is_truthy(src:match("checkbox = {"),
			"checkbox config block missing")
		assert.is_truthy(src:match("unchecked = {"),
			"unchecked checkbox icon config missing")
		assert.is_truthy(src:match("checked%s+= {"),
			"checked checkbox icon config missing")
	end)
